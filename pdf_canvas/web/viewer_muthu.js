  ...
  this.draw = function pageviewDraw(callback) {
    ...
    if (!pdfPage) {
      ...
      this.pagePdfPromise = promise;
        parent.highlight_pdf(window.frameElement.id)
      return;
    }
...
var PDFViewerApplication = {
  ...
  setHash: function pdfViewSetHash(hash) {
    ...
    if (hash.indexOf('=') >= 0) {
      ...
      if('doc_period' in params){
            document.querySelector('#period').innerHTML = params['doc_period']
      }else{
            document.querySelector('#period').innerHTML = ''
        }
        try{
            if(window.gbl_bbox){
                highlightsbbox(1, window.gbl_bbox);
            }
            console.log('bbox ', params['bbox'])
            var bbox    = JSON.parse(JSON.parse(params['bbox']))
            window.gbl_bbox = bbox;
            console.log('bbox ',bbox)
            highlightsbbox(1, bbox);
        }catch(e){
                console.log('Error ', e, params['bbox'])
            }
  ...
  setScale: function (value, resetAutoSettings) {
		    ...
		    var eov      = this.pdfViewer.linkService.pdfViewer.linkService.pdfViewer.currentScale;
		    var cur_scl  = Math.round(eov * 10000) / 100;
		    var cur_res  = cur_scl/100*0.5;
		    var zm_scl   = 1/(cur_scl*(0.01333));
		    var z_scl    = zm_scl.toFixed(2);
		    var bbox_ref=[];
		    try{
			    var cls_dm         = document.getElementsByClassName('NITIN');
			    var cls_dm_len     = cls_dm.length;
			    if(cls_dm_len != 0){
				    for(var r=0;r<cls_dm_len;r++){
					    var each_ref    =  cls_dm[r].getAttribute('ref_ids');
					    bbox_ref.push(each_ref);
				    }
				    for(var j=0;j<cls_dm_len;j++){
					    cls_dm[cls_dm.length-1].parentNode.removeChild(cls_dm[cls_dm.length-1]);
				    }
			    }
		    }catch(e){
		    }
		    var ref_zip = bbox_ref.join('#');
		    //parent.highlight_pdf(ref_zip,z_scl);
  },
...
window.highlightsbbox = function(page, bboxs,id){
    //var bboxs = [[368, 723, 30, 6]] //[[342, 723, 56, 6]] //[[257, 542, 42, 5]]
    //var bboxs = [[408.44444444444446, 429.3333333333333, 25.333333333333332, 6.666666666666667]] //[[342, 723, 56, 6]] //[[257, 542, 42, 5]]
    console.log('new co ', bboxs,id)
    //var page= '70'
    //console.log('sl',id)
    var content_window = window; //document.querySelector('#'+id).contentWindow
    ...
    bboxs.forEach(function(bbox){
        ...
        //var bbox           = bbox.split('_')//.map(function(t){ return Number(t)})
        //var bbox           = bbox.map(function(t){ return parseInt(Number(t))})
        var bbox           = bbox.map(function(t){ return Number(t)})
        var left           = (bbox[0]*1.25) //+(bbox[0]/2)
        var top1           = ((bbox[1]-3)*1.25) //+(bbox[1]/2)
        var width          = (bbox[2]*1.25) //+(bbox[2]/2)
        var height         = ((bbox[3]+3)*1.25) //+(bbox[3]/2)
        var style          = 'width:'+width+'px;height:'+height+'px;top:'+top1+'px;left:'+left+'px;z-index:100000000;background-color: rgba(243, 230, 88, 0.407843);border: 2px solid rgb(0, 126, 255);position: absolute;box-sizing: content-box;'
        var nw_som  =   create_dom('div',{'style': style,'role':'tas_highlights'},p_dom,'')
        console.log(bbox, nw_som)
        if(d==0 && nw_som){
            d = 1
            ...
            if((left+width) > w_width){
                    ...
                    console.log( 'ssss ' ,sleft )
            }
            console.log('w , sl , l ', w_width , sleft , left ,top1)
            //nw_som.scrollIntoView()
            $(content_window.document.querySelector('#viewerContainer')).animate({
                scrollTop: ((top1+height) - 150),
                scrollLeft: sleft,
            });
            //content_window.scrollTo($(nw_som).parent().offset().left,$(nw_som).parent().offset().top);
        }
    })
}
